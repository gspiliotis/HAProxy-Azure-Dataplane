...
class TestGetConfigurationVersion:
    ...
    def test_returns_version(self, client):
        responses.add(responses.GET, f"{BASE}/services/haproxy/configuration/version", body="42")
...
class TestTransactions:
    ...
    def test_create_transaction(self, client):
        responses.add(
            responses.POST, f"{BASE}/services/haproxy/transactions",
    ...
    def test_commit_transaction(self, client):
        responses.add(responses.PUT, f"{BASE}/services/haproxy/transactions/txn-1", status=200)
        client.commit_transaction("txn-1")  # Should not raise

    @responses.activate
    def test_commit_version_conflict(self, client):
        responses.add(
            responses.PUT, f"{BASE}/services/haproxy/transactions/txn-1",
    ...
    def test_delete_transaction(self, client):
        responses.add(responses.DELETE, f"{BASE}/services/haproxy/transactions/txn-1", status=200)
...
class TestBackends:
    ...
    def test_list_backends(self, client):
        responses.add(
            responses.GET, f"{BASE}/services/haproxy/configuration/backends",
            json={"data": [{"name": "b1"}]},
        )
    ...
    def test_get_backend_found(self, client):
        responses.add(
            responses.GET, f"{BASE}/services/haproxy/configuration/backends/b1",
    ...
    def test_get_backend_not_found(self, client):
        responses.add(
            responses.GET, f"{BASE}/services/haproxy/configuration/backends/missing",
    ...
    def test_create_backend(self, client):
        responses.add(
            responses.POST, f"{BASE}/services/haproxy/configuration/backends",
...
class TestServers:
    ...
    def test_list_servers(self, client):
        responses.add(
            responses.GET, f"{BASE}/services/haproxy/configuration/servers",
            json={"data": [{"name": "srv1"}]},
        )
    ...
    def test_create_server(self, client):
        responses.add(
            responses.POST, f"{BASE}/services/haproxy/configuration/servers",
    ...
    def test_replace_server(self, client):
        responses.add(
            responses.PUT, f"{BASE}/services/haproxy/configuration/servers/srv1",
...
class TestErrorHandling:
    ...
    def test_generic_error(self, client):
        responses.add(
            responses.GET, f"{BASE}/services/haproxy/configuration/version",
